...
public class ExceptionHandlersTest {
	...
	public void handlePoductstoreExceptionTest() {
		//GIVEN
		PoductstoreException ex= new PoductstoreException("", HttpStatus.BAD_REQUEST);
		Mockito.when(builderReplyOnException.replyOnException(Mockito.any(PoductstoreException.class), Mockito.any(HttpStatus.class), Mockito.any(WebRequest.class))).thenReturn(replyError);
